...
def concat(f1, f2, output_folder):
    ...
    for k in data2["utts"]:
        if k in data1["utts"]:
            #data1["utts"]['dup-'+k] = data2["utts"][k]
            continue
        else:
            data1["utts"][k] = data2["utts"][k]
    with open(os.path.join(output_folder, "data_concat.json"), 'w',encoding='utf8') as f:
        json.dump(data1, f, indent=4,ensure_ascii=False)
    print("New json stored in {} with {} utterances".format(os.path.join(output_folder, "data_concat.json"), len(data1['utts'])))
